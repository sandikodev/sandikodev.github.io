...
export class WebNativeBridge implements NativeBridge {
  ...
  async createWindow(config: WindowConfig): Promise<NativeWindowHandle> {
    ...
    if (config.antiCapitalist) {
      console.log('💀 Anti-capitalist mode enabled - No telemetry, no tracking');
    }
  ...
  private calculatePerformanceLevel(info: SystemInfo): SystemInfo['performance'] {
    if (info.os === 'redox') return 'ludicrous';
    if (info.os === 'linux' && info.compositor === 'wayland') return 'ludicrous';
